...
// Delete Warehouse
func (u *Warehouse) Delete(ctx context.Context, in *inventories.Id) (*inventories.Boolean, error) {
...
func (u *Warehouse) List(in *inventories.ListWarehouseRequest, stream inventories.WarehouseService_ListServer) error {
	...
	for rows.Next() {
		...
		pbWarehouse.CreatedAt, err = ptypes.TimestampProto(createdAt)
		if err != nil {
			return status.Errorf(codes.Internal, "convert createdAt: %v", err)
		}

		pbWarehouse.UpdatedAt, err = ptypes.TimestampProto(updatedAt)
		if err != nil {
